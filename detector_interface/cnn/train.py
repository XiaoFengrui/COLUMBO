...
def get_gpu_memory_info():
    ...
    for i in range(device_count):
        ...
        print(f"GPU {i}: {mem_info.used / (1024 ** 2):.2f} MB used, {mem_info.total / (1024 ** 2):.2f} MB total")
...
def main():
    ...
    train_df = pd.read_csv('/home/ustc-5/XiaoF/AdvWebDefen/Dataset/' + dataset + '/train.tsv', delimiter='\t', names=['text', 'label'], skiprows=1)
    ...
    word2idx = load_word2idx('/home/ustc-5/XiaoF/AdvWebDefen/AutoSpear-main/classifiers/repos/cnn/word2idx.json')
    ...
    # 初始化模型
    model, optimizer = initilize_cnn_model(device, vocab_size=len(word2idx), embed_dim=300, dropout=0.5, learning_rate=0.01)
    ...
    for epoch in range(N_EPOCHS):
        ...
        # _, _ = get_process_gpu_usage()
        valid_loss, valid_acc = evaluate(model, val_loader, criterion)

        print(f'\tTrain Loss: {train_loss:.3f} | Train Acc: {train_acc*100:.2f}%')
        print(f'\t Val. Loss: {valid_loss:.3f} |  Val. Acc: {valid_acc*100:.2f}%')
        if valid_acc > best_acc:
            # torch.save(model.state_dict(), './cnn_model_' + dataset + '.pth')
            torch.save(model.state_dict(), './cnn_model_' + dataset + '_%d.pth'%(training_ratio))
    print('Training Time: ', time.time() - t0)
    
    t1 = time.time()
    print("... Testing ...")
    model, _ = initilize_cnn_model(device, vocab_size=len(word2idx), embed_dim=300, dropout=0.5)
    # model.load_state_dict(torch.load('./cnn_model_' + dataset + '.pth'))
    model.load_state_dict(torch.load('./cnn_model_' + dataset + '_%d.pth'%(training_ratio)))
