...
from utils.misc.misc_utils import read_payloads, read_payloads_csv
...
def main():
    ...
    payloads, labels = read_payloads_csv(payload_path)
    ...
    detector = input_args.detector
    tree_mutate_pattern = input_args.tree_mutate_pattern

    fbenign = open('{}/{}-{}-{}#benign.log'.format(log_path, detector, request_method, guide), 'a+')
    fexcept = open('{}/{}-{}-{}#except.log'.format(log_path, detector, request_method, guide), 'a+')
    fsummary = open('{}/{}-{}-{}#summary.log'.format(log_path, detector, request_method, guide), 'a+')
    ...
    for idx, payload in tqdm(enumerate(payloads)):
        run_res = _run(idx, attacker, payload, log_path, clsf=clsf, guide=guide, max_attempts=max_attempts,
                       max_steps=max_steps, detector=detector, request_method=request_method,blackbox=blackbox, tree_mutate_pattern=tree_mutate_pattern)
        if run_res['benign']:
...
def _run(idx, attacker, payload, log_path, clsf, guide, max_attempts, max_steps, detector, request_method, blackbox, tree_mutate_pattern):
    fdetail = open('{}/{}-{}-{}#detail.log'.format(log_path, detector, request_method, guide), 'a+')
    fsuccess = open('{}/{}-{}-{}#succes.log'.format(log_path, detector, request_method, guide), 'a+')
    ...
    if init_score <= threshold:
        ...
        return run_res
    print(idx, 'Initial Score: ', init_score, 'Initial Payload: ', payload, file=fdetail, flush=True)
