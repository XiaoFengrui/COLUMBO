...
import sys
sys.path.append('/home/ustc-5/XiaoF/AdvWebDefen/AdvSqli/mutate/utils/')
...
class Attacker:
    ...
    def mcts_attack(self, idx, attempt, max_attempts, fdetail, fsuccess, tree, clsf, init_score, max_steps=6, computation_budget=10, blackbox=True, tree_mutate_pattern='all'):
        """
        ...
        try:
            ...
            for stp in range(max_steps):
                ...
                if score < min_score:
                    min_score = score
                    min_score_payload = output
                if score <= clsf.get_thresh(): # 小于阈值，成功欺骗，标记成功
                    print(idx, '\t', (origin_output+" ").encode(), file=fsuccess, flush=True)
                    print(idx, '\t', (output+" ").encode(), file=fsuccess, flush=True)
