...
from DeBERTa.utils import get_logger, boolean_string
...
class RTDModel(NNModule):
    def __init__(self, config, *wargs, **kwargs):
        ...
        self.register_discriminator_fw_hook()  # share embedding
...
class RTDTask(Task):
        ...
        def train_fn(args, model, device, data_fn, eval_fn, loss_fn):
            if args.decoupled_training:
                ...
                trainer = DistributedTrainer(args, args.output_dir, model, device, data_fn, loss_fn=loss_fn,
                                             eval_fn=eval_fn, dump_interval=args.dump_interval)
            trainer.train()
    ...
    def get_decoupled_loss_fn(self, args, model, data_fn, device, num_training_steps):
        ...
        disc_trainer = DistributedTrainer(disc_args, args.output_dir, model.discriminator, device, data_fn,
                                          loss_fn=d_loss_fn, eval_fn=eval_fn, dump_interval=args.dump_interval,
                                          name='D')
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument('--decoupled_training', type=boolean_string, default=True,
                            help='Whether to use decoupled training')
        parser.add_argument('--num_training_steps', type=int, default=None, help='Maxium pre-training steps')
        parser.add_argument('--discriminator_learning_rate', type=float, default=-1,
