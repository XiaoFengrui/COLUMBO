...
from DeBERTa.data import DynamicDataset
    ...
    def eval_data(self, max_seq_len=512, dataset_size=None, **kwargs):
        ...
        for d in ds:
            if dataset_size is None:
                ...
                _size = dataset_size
            d.data = DynamicDataset(d.data, feature_fn=self.get_feature_fn(max_seq_len=max_seq_len, label_type='float'),
...
class MRPCTask(Task):
    ...
    def train_data(self, max_seq_len=512, dataset_size=None, epochs=1, mask_gen=None, **kwargs):
        ...
        data = self._read_tsv(input_src)
        examples = ExampleSet(
            [ExampleInstance((l[3], l[4]), self.label2id(l[0])) for l in data[1:]])  # if l[3] in ['slate']])
        if dataset_size is None:
            dataset_size = len(examples) * epochs
        return DynamicDataset(examples, feature_fn=self.get_feature_fn(max_seq_len=max_seq_len, mask_gen=mask_gen), \
                              dataset_size=dataset_size, shuffle=True, **kwargs)
...
class ColaTask(Task):
    ...
    def train_data(self, max_seq_len=512, dataset_size=None, epochs=1, mask_gen=None, **kwargs):
        ...
        if dataset_size is None:
            dataset_size = len(examples) * epochs
        return DynamicDataset(examples, feature_fn=self.get_feature_fn(max_seq_len=max_seq_len, mask_gen=mask_gen,
                                                                       label_type='float', training=True), \
                              dataset_size=dataset_size, shuffle=True, **kwargs)
    ...
    def eval_data(self, max_seq_len=512, dataset_size=None, **kwargs):
        ...
        for d in ds:
            if dataset_size is None:
                ...
                _size = dataset_size
            d.data = DynamicDataset(d.data, feature_fn=self.get_feature_fn(max_seq_len=max_seq_len, label_type='int'),
                                    dataset_size=_size, **kwargs)
        return ds
...
class MNLITask(Task):
    ...
    def _data(self, name, path, type_name='dev', ignore_metric=False):
        ...
        return EvalData(name, examples,
                        metrics_fn=self.get_metrics_fn(input_src), predict_fn=predict_fn, ignore_metric=ignore_metric,
                        critial_metrics=['accuracy'])
