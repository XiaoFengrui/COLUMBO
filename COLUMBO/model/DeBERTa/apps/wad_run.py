...
from ..sift import AdversarialLearner, hook_sift_layer, GenerativeAdversarialLearner
...
def train_model(args, model, device, train_data, eval_data, adv_eval_data, run_eval_fn, train_fn=None, loss_fn=None):
    total_examples = len(train_data)
    if args.num_train_steps > 0:
        num_train_steps = args.num_train_steps
    else:
        # num_train_steps = int(len(train_data) * args.num_train_epochs / args.train_batch_size)
        num_train_steps = (len(train_data) // args.train_batch_size)*args.num_train_epochs 
    ...
    def eval_fn(trainer, model, device, tag):
        results = run_eval_fn(trainer.args, model, device, eval_data, tag,
                              steps=trainer.trainer_state.steps)
        eval_metric = np.mean([v[0] for k, v in results.items() if 'train' not in k])
        return eval_metric

    def adv_eval_fn(trainer, model, device, tag):
        results = run_eval_fn(trainer.args, model, device, adv_eval_data, tag,
                              steps=trainer.trainer_state.steps)
        eval_metric = np.mean([v[0] for k, v in results.items() if 'train' not in k])
        return eval_metric
    ...
    def get_adv_loss_fn():
        adv_modules = hook_sift_layer(model, hidden_size=model.config.hidden_size, learning_rate=args.vat_learning_rate,
                                      init_perturbation=args.vat_init_perturbation)
        # adv = AdversarialLearner(model, adv_modules)
        adv = GenerativeAdversarialLearner(model, adv_modules)

        def adv_loss_fn(trainer, model, data):
            output = model(**data)
            logits = output['logits']
            loss = output['loss'] # eq.7
            if isinstance(logits, Sequence):
                logits = logits[-1]

            if args.vat_lambda > 0:
                def pert_logits_fn(model, **data): # eq.8
                    o = model(**data)
                    logits = o['logits']
                    if isinstance(logits, Sequence):
                        logits = logits[-1]
                    return logits, o['mask_loss']

                gat_loss, rtd_loss = adv.loss(logits, pert_logits_fn, loss, **data)
                # loss += gat_loss * args.vat_lambda
                if args.rtd_weight > 0:
                    loss += rtd_loss * args.rtd_weight

            return loss.mean(), data['input_ids'].size(0)

        return adv_loss_fn

    def get_rtd_loss_fn(trainer, model, data):
        output = model(**data)
        logits = output['logits']
        loss = output['loss']
        rand = random.Random(0)
        new_data, mlm_loss, gen_output = model.make_electra_data(input_data=data, rand=rand)
        output = model(**new_data)
        loss += args.rtd_weight * output['mask_loss'].mean()

        return loss.mean(), data['input_ids'].size(0)

    def _train_fn(args, model, device, data_fn, eval_fn, loss_fn):

        # 如果传入了正的 vat_lambda 参数，使用 adv loss，其余使用普通 loss
        if loss_fn is None:
            loss_fn = get_adv_loss_fn() if args.vat_lambda > 0 else _loss_fn
            if args.rtd_weight > 0 and args.vat_lambda == 0:
                loss_fn = get_rtd_loss_fn
                import pdb
                pdb.set_trace()
        trainer = DistributedTrainer(args, args.output_dir, model, device, data_fn, loss_fn=loss_fn, eval_fn=eval_fn,
                                     adv_eval_fn=adv_eval_fn, dump_interval=args.dump_interval)
        trainer.train()
        ...
        for batch in tqdm(AsyncDataLoader(eval_dataloader), ncols=80, desc='Evaluating: {}'.format(prefix),
            ...
            if args.export_onnx_model:
                if ort_session is None:
                    ...
                    ort_session = ort.InferenceSession(ort_model, sess_options=sess_opt,
...
def main(args):
    ...
    vocab_path, vocab_type = load_vocab(vocab_path=args.vocab_path, vocab_type=args.vocab_type,
    ...
    task = get_task(args.task_name)(tokenizer=tokenizer, args=args, max_seq_len=args.max_seq_length,
                                    data_dir=args.data_dir)
    # exit(0)
    label_list = task.get_labels()

    eval_data = task.eval_data(max_seq_len=args.max_seq_length)
    adv_eval_data = task.adv_eval_data(name=args.task_name, path=args.adv_data_path, max_seq_len=args.max_seq_length)
    ...
    if args.do_eval:
        # checkpoint_path = '/home/ustc-5/XiaoF/AdvWebDefen/GenerAT-main/output/deberta-v3-large/SIK/pytorch.model-003060.bin'
        checkpoint_path = '/home/ustc-5/XiaoF/AdvWebDefen/GenerAT-main/output/deberta-v3-large/HPD/ablation/woRTD/pytorch.model-001562.bin'
        state_dict = torch.load(checkpoint_path, map_location = device)  # 或者 'cuda' 如果你想在 GPU 上加载
        model.discriminator.load_state_dict(state_dict)

        run_eval(args, model, device, test_data, prefix=args.tag)

    if args.do_train:
        train_fn = task.get_train_fn(args, model)
        train_model(args, model, device, train_data, eval_data, adv_eval_data, run_eval_fn, loss_fn=loss_fn,
                    train_fn=train_fn)
