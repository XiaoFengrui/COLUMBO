...
def download_asset(url, name, tag=None, no_cache=False, cache_dir=None):
    ...
    try:
        with open(output, 'wb') as fs:
            progress = tqdm(total=int(resp.headers['Content-Length']) if 'Content-Length' in resp.headers else -1,
...
def load_model_state(path_or_pretrained_id, tag=None, no_cache=False, cache_dir=None):
    ...
    if model_path and (not os.path.exists(model_path)) and (path_or_pretrained_id.lower() in pretrained_models):
        ...
        if (not os.path.exists(model_path)) or no_cache:
            asset = download_asset(pretrained.model_url, 'pytorch_model.bin', tag=tag, no_cache=no_cache,
                                   cache_dir=cache_dir)
            asset = download_asset(pretrained.config_url, 'model_config.json', tag=tag, no_cache=no_cache,
                                   cache_dir=cache_dir)
    elif not model_path:
        return None, None
