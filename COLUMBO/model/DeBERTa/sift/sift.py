...
__all__ = ['PerturbationLayer', 'AdversarialLearner', 'hook_sift_layer', 'GenerativeAdversarialLearner']
...
def hook_sift_layer(model, hidden_size, learning_rate=1e-4, init_perturbation=1e-2,
                    target_module='discriminator.deberta.embeddings.LayerNorm'):
    """
...
class AdversarialLearner:
    ...
    def loss(self, target, logits_fn, loss_fn='symmetric-kl', *wargs, **kwargs):
        ...
        pert_loss.backward()  # 获得梯度
        for m in self.adv_modules:
            ok = m.update_delta(True)  # 梯度累加在 delta 上
        # 改这个 loss，修改为 mlm 生成样本
        # 这里获得对抗梯度后，下面 mlm 对 data 生成 new data
        # 然后计算 symetric_kl
        for r, p in zip(self.prev, self.parameters):
            p.requires_grad_(r)

        pert_logits = logits_fn(self.model, *wargs, **kwargs)  # 有对抗梯度的前传
...
class GenerativeAdversarialLearner:
    """ Adversarial Learner
    This class is the helper class for adversarial training.
    ...
    def loss(self, target, logits_fn, loss, *wargs, **kwargs):
        """
        Calculate the adversarial loss based on the given logits fucntion and loss function.
        Inputs:
        `target`: the logits from original inputs.
        ...
        new_data, mlm_loss, gen_output = self.model.make_electra_data(input_data=kwargs, rand=rand)
        kwargs = new_data

        self.cleanup()
        self.model.zero_grad()

        pert_logits, mask_loss = logits_fn(self.model, *wargs, **kwargs)
        pert_loss = symmetric_kl(pert_logits, target) # eq.9

        return pert_loss.mean(), mask_loss.mean()
