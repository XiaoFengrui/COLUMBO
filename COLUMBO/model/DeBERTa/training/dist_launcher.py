...
from DeBERTa.utils import set_logger, get_logger
...
def _setup_distributed_group(args):
    ...
    if args.world_size == 1:
        ...
        set_logger(args.task_name, os.path.join(args.output_dir, f'training_{args.task_name}_{args.rank}.log'),
                   rank=args.rank, verbose=1 if args.local_rank == 0 else 0)
    ...
    logger.info("device=%s, n_gpu=%d, distributed training=%r, world_size=%d", device, n_gpu,
                bool(args.world_size != 1), args.world_size)
    return device
...
def test_dist_launch():
    def test_functions(args):
        global logger
        set_logger(args.task_name, os.path.join(args.output_dir, f'training_{args.task_name}_{args.node_rank}.log'),
                   rank=args.rank)
        logger.info(args)
