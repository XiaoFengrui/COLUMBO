...
class GenerativeAdversarialTrainingModel(NNModule):
    # 钩子函数是一种允许用户在某个事件或操作发生前后插入自定义代码的机制
    def __init__(self, config, num_labels=2, drop_out=None, pre_trained=None):
        ...
        self.discriminator._register_load_state_dict_pre_hook(self._pre_load_hook) # 当使用model.load_state_dict(state_dict)方法加载模型参数时，PyTorch会在实际加载参数之前调用self._pre_load_hook函数
    ...
    def _pre_load_hook(self, state_dict, prefix, local_metadata, strict,
                       missing_keys, unexpected_keys, error_msgs): #将bert前缀的参数名替换为 deberta 前缀，使得 BERT 模型的参数可以加载到 DeBERTa 模型中
        new_state = dict()
    ...
    def register_discriminator_fw_hook(self, *wargs): # 将判别器和生成器的嵌入层参数同步
        ...
        self.discriminator.register_forward_pre_hook(fw_hook) # 每次前向传播就会运行hook函数
