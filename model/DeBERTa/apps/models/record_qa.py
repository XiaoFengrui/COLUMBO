...
class ReCoRDQAModel(NNModule):
    ...
    def forward(self, input_ids, entity_indice, type_ids=None, input_mask=None, labels=None, position_ids=None,
        ...
        # bxexspxd
        entities = torch.gather(tokens.unsqueeze(1).expand(entity_indice.size()[:2] + tokens.size()[1:]),
                                index=entity_indice.long().unsqueeze(-1).expand(
                                    entity_indice.size() + (tokens.size(-1),)), dim=-2)
        ctx = tokens[:, :1, :] / math.sqrt(tokens.size(-1))
        # bxsx1
        att_score = torch.matmul(tokens, ctx.transpose(-1, -2))
        # bxexspx1
        entity_score = torch.gather(att_score.unsqueeze(1).expand(entity_indice.size()[:2] + att_score.size()[1:]),
                                    index=entity_indice.long().unsqueeze(-1).expand(
                                        entity_indice.size() + (att_score.size(-1),)), dim=-2)
        entity_score = entity_score.squeeze(-1) * entity_mask.to(entity_score) - (
