...
class EnhancedMaskDecoder(torch.nn.Module):
    ...
    def forward(self, ctx_layers, ebd_weight, target_ids, input_ids, input_mask, z_states, attention_mask, encoder,
                relative_pos=None):
        mlm_ctx_layers = self.emd_context_layer(ctx_layers, z_states, attention_mask, encoder, target_ids, input_ids,
                                                input_mask, relative_pos=relative_pos)
    ...
    def emd_context_layer(self, encoder_layers, z_states, attention_mask, encoder, target_ids, input_ids, input_mask,
                          relative_pos=None):
...
class MaskedLanguageModel(NNModule):
    ...
    def __init__(self, config, *wargs, **kwargs):
        ...
        self.lm_predictions = EnhancedMaskDecoder(self.deberta.config,
                                                  self.deberta.embeddings.word_embeddings.weight.size(0))
        self.apply(self.init_weights)

    def forward(self, input_ids, input_mask=None, lm_labels=None, position_ids=None, attention_mask=None, type_ids=None, dis_labels=None, labels=None, **kwargs):
        ...
        if lm_labels is not None:
            ...
            if label_index.size(0) > 0:
                (lm_logits, lm_labels, lm_loss) = self.lm_predictions(encoder_layers, ebd_weight, lm_labels, input_ids,
                                                                      input_mask, z_states, attention_mask,
