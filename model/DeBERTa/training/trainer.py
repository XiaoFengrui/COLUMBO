...
from DeBERTa.data import BatchSampler, DistributedBatchSampler, RandomSampler, AsyncDataLoader
...
class TrainerState:
    def __init__(self, training_steps, name=None):
        ...
        self.name = name
        self.run_id = None
        self.adv_best_steps = 0
        self.adv_best_metric = -1e9
    ...
    def report_state(self):
        ...
        if self.name is not None:
            ...
            tag = None
        logger.info('{}[{:0.1f}%][{:0.2f}h] Steps={}, loss={}, examples={}, loss_scale={:0.1f}, {:0.1f}s'.format(tag,
...
class DistributedTrainer:
    def __init__(self, args, output_dir, model, device, data_fn, loss_fn=None, optimizer_fn=None, eval_fn=None,
                 adv_eval_fn=None, init_fn=None, update_fn=None, dump_interval=10000, name=None, **kwargs):
        """
        ...
        if training_steps is None:
            training_steps = getattr(args, 'training_steps', (
                        len(training_data) + self.args.train_batch_size - 1) // self.args.train_batch_size * self.training_epochs)
    ...
    def train(self):
        ...
        for n_epoch in range(self.trainer_state.epochs, self.training_epochs):
            ...
            self._eval_model()  # 测一次 eval 上的性能
            self._adv_eval_model()  # 测一次 adv eval 上的性能
    ...
    def _eval_model(self, with_checkpoint=True):
        ...
        if self.eval_fn is not None:
            metric = self.eval_fn(self, self.model, self.device,
    ...
    def _adv_eval_model(self, with_checkpoint=True):
        if with_checkpoint:
            checkpoint_dir = getattr(self.args, 'checkpoint_dir', None)
            checkpoint_dir = checkpoint_dir if checkpoint_dir is not None else self.output_dir
            chk_postfix = f'{self.trainer_state.steps:06}'
            self.save_model(self.args, checkpoint_dir, chk_postfix, self.model, self.optimizer)
        _metric = self.trainer_state.adv_best_metric
        _steps = self.trainer_state.adv_best_steps
        if self.adv_eval_fn is not None:
            metric = self.adv_eval_fn(self, self.model, self.device,
                                      tag=f'{self.trainer_state.steps:06}-{self.training_steps}')
            if metric > _metric:
                _metric = metric
                _steps = self.trainer_state.steps
            logger.info(f'Best metric: {_metric}@{_steps}')
        self.trainer_state.adv_best_metric, self.trainer_state.adv_best_steps = _metric, _steps
